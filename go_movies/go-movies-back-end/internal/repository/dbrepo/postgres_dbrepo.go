...
func (m *PostgresDBRepo) GetOneMovie(id int) (*models.Movie, error) {
	...
	// get genre
	query = `select g.id, g.genre from movies_genres as mg left join genres g on (mg.genre_id = g.id) where mg.movie_id = $1 order by g.genre`
...
func (m *PostgresDBRepo) GetOneMovieEdit(id int) (*models.Movie, []*models.Genre, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeOut)
	defer cancel()
...
func (m *PostgresDBRepo) InsertMovie(movie models.Movie) (int, error) {
	...
	query := `insert into movies (title, description, release_date, runtime, mpaa_rating, created_at, updated_at, image) values ($1, $2, $3, $4, $5, $6, $7, $8) returning id`
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	query := `update movies set title = $1, description = $2, release_date = $3,
